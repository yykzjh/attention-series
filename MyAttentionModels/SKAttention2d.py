...
class SKAttention2d(nn.Module):
    ...
    def __init__(self, channel=512, kernels=[1, 3, 5, 7], r=16, group=1, L=32):
        ...
        :param L: 通道数衰减最小值
        """
        super(SKAttention2d, self).__init__()
        # 计算通道数衰减后的数值
    ...
    def forward(self, x):
        ...
        attention_weights = self.softmax(attention_weights)  # k, bs, c, 1, 1
        # 将之前不同分支卷积得到的结果堆叠起来
        features = torch.stack(conv_outs, dim=0)  # k, bs, c, h, w
...
if __name__ == '__main__':
    ...
    model = SKAttention2d(channel=256)
