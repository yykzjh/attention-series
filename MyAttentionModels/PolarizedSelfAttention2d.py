...
class ParallelPolarizedSelfAttention2d(nn.Module):
    ...
    def forward(self, x):
        ...
        sp_Wv = self.sp_Wv(x)  # bs,c//2,h,w
        sp_Wq = self.sp_Wq(x)  # bs,c//2,h,w
        sp_Wq = self.avg_pool(sp_Wq)  # bs,c//2,1,1
        sp_Wv = sp_Wv.reshape(bs, c//2, -1)  # bs,c//2,h*w
        sp_Wq = sp_Wq.reshape(bs, 1, c//2)  # bs,1,c//2
        sp_Wq = self.sp_softmax(sp_Wq)  # bs,1,c//2
        sp_Wz = torch.matmul(sp_Wq, sp_Wv)  # bs,1,h*w
        sp_score = self.sigmoid(sp_Wz.reshape(bs, 1, h, w))  # bs,1,h,w
        sp_out = sp_score * x
...
class SequentialPolarizedSelfAttention2d(nn.Module):
    """
    串行极化自注意力
    """
    def __init__(self, channel=512):
        ...
        :param channel: 输入特征图的通道数
        """
        super(SequentialPolarizedSelfAttention2d, self).__init__()
        # 定义通道自注意的两个1X1卷积Wv、Wq
        self.ch_Wv = nn.Conv2d(channel, channel // 2, kernel_size=1)
        self.ch_Wq = nn.Conv2d(channel, 1, kernel_size=1)
        # 定义通道注意力的Softmax
    ...
    def forward(self, x):
        ...
        # Spatial-only Self-Attention
        sp_Wv = self.sp_Wv(ch_out)  # bs,c//2,h,w
        sp_Wq = self.sp_Wq(ch_out)  # bs,c//2,h,w
        sp_Wq = self.avg_pool(sp_Wq)  # bs,c//2,1,1
        sp_Wv = sp_Wv.reshape(bs, c//2, -1)  # bs,c//2,h*w
        sp_Wq = sp_Wq.reshape(bs, 1, c//2)  # bs,1,c//2
        sp_Wq = self.sp_softmax(sp_Wq)  # bs,1,c//2
...
if __name__ == '__main__':
    model = ParallelPolarizedSelfAttention2d(channel=256)
    model1 = SequentialPolarizedSelfAttention2d(channel=256)

    x = torch.rand((4, 256, 112, 112))
